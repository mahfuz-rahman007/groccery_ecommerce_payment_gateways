        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card card-primary card-outline">
                    ...
                    <div class="card-body">
                        <form class="form-horizontal" action="{{ route('admin.sliderUpdate', $slider->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                                ...
                                <div class="col-sm-10">
                                    <select name="pcategory_id" class="form-control">
                                        @foreach ($pcategories as $pcategory)
                                            <option value="{{ $pcategory->id }}" {{ $pcategory->id == $slider->pcategory_id ? 'selected':'' }}>{{ $pcategory->name }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" name="name" id="name" class="form-control" placeholder="{{ __('Name') }}" value="{{ $slider->name }}">
                                    @if($errors->has('name'))
